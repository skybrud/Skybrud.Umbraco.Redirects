...
public class RedirectsService : IRedirectsService {
    ...
    public IRedirect AddRedirect(AddRedirectOptions options) {
        ...
        // Initialize a new redirect DTO
        RedirectDto dto = new() {
            Key = Guid.NewGuid(),
            DestinationType = nameof(RedirectDestinationType.Url),
        ...
        // Persist the redirect to the database
        scope.Complete();
    ...
    public IRedirect? GetRedirectById(int id) {
    ...
    public IReadOnlyList<IRedirect> GetRedirects() {
        ...
        // Make the call to the database
        IReadOnlyList<IRedirect> redirects = scope.Database
            .Fetch<RedirectDto>(sql)
            .SelectList(Redirect.CreateFromDto);
    ...
    public RedirectsSearchResult GetRedirects(RedirectsSearchOptions options) {
        ...
        // Order the redirects
        sql = sql.OrderByDescending<RedirectDto>(x => x.UpdateDate);
        ...
        IReadOnlyList<IRedirect> items = all
            .Skip(offset)
            .Take(limit)
            .SelectList(x => (IRedirect) Redirect.CreateFromDto(x));
    ...
    /// <summary>
    /// Returns a list of redirects where the destination matches the specified <paramref name="nodeType"/> and <paramref name="nodeId"/>.
    /// </summary>
    /// <param name="nodeType">The type of the destination node.</param>
    /// <param name="nodeId">The numeric ID of the destination node.</param>
    /// <returns>A list of <see cref="IRedirect"/>.</returns>
    public virtual IReadOnlyList<IRedirect> GetRedirectsByNodeId(RedirectDestinationType nodeType, int nodeId) {
    ...
    /// <summary>
    /// Returns a list of redirects where the destination matches the specified <paramref name="nodeType"/>, <paramref name="nodeId"/> and <paramref name="culture"/>.
    /// </summary>
    /// <param name="nodeType">The type of the destination node.</param>
    /// <param name="nodeId">The numeric ID of the destination node.</param>
    /// <param name="culture">The culture the returned redirects should match.</param>
    /// <returns>A list of <see cref="IRedirect"/>.</returns>
    public IReadOnlyList<IRedirect> GetRedirectsByNodeId(RedirectDestinationType nodeType, int nodeId, string? culture) {
        IReadOnlyList<IRedirect> redirects = GetRedirectsByNodeId(nodeType, nodeId);
    ...
    /// <summary>
    /// Returns a list of redirects where the destination matches the specified <paramref name="nodeType"/> and <paramref name="nodeKey"/>.
    /// </summary>
    /// <param name="nodeType">The type of the destination node.</param>
    /// <param name="nodeKey">The key (GUID) of the destination node.</param>
    /// <returns>A list of <see cref="IRedirect"/>.</returns>
    public virtual IReadOnlyList<IRedirect> GetRedirectsByNodeKey(RedirectDestinationType nodeType, Guid nodeKey) {
        if (nodeType == RedirectDestinationType.Url) throw new RedirectsException($"Unsupported node type: {nodeType}");
    ...
    /// <param name="culture">The culture the returned redirects should match.</param>
    /// <returns>A list of <see cref="IRedirect"/>.</returns>
    public IReadOnlyList<IRedirect> GetRedirectsByNodeKey(RedirectDestinationType nodeType, Guid nodeKey, string? culture) {
    ...
    /// <param name="nodeId">The numeric ID of the destination node.</param>
    /// <returns>A list of <see cref="IRedirect"/>.</returns>
    protected virtual IReadOnlyList<IRedirect> GetRedirectsByNodeId(string nodeType, int nodeId) {
    ...
    public IRedirect SaveRedirect(IRedirect redirect) {
        ...
        try {
            ...
            throw new RedirectsException($"Failed updating redirect with key '{redirect.Key}'.", ex);
        }
    ...
    public virtual void DeleteRedirect(IRedirect redirect) {
        ...
        try {
            ...
            throw new RedirectsException($"Failed deleting redirect with key '{redirect.Key}.", ex);
        }
    ...
    public IReadOnlyList<RedirectDomain> GetDomains() {
        return _domains
            .GetAllAsync(false).Result
            .Select(RedirectDomain.GetFromDomain)
            .ToArray()!;
    }
    ...
    public IReadOnlyList<RedirectRootNode> GetRootNodes() {

        // Multiple domains may be configured for a single node, so we need to group the domains before proceeding
        IEnumerable<IGrouping<int, RedirectDomain>> domainsByRootNodeId = GetDomains().GroupBy(x => x.RootNodeId);
    ...
    /// <param name="domain">When this method returns, holds an instance of <see cref="Domain"/> representing the matched domain if successful; otherwise, <see langword="null"/>.</param>
